...
load_diabetes=pickle.load(open("D:/Deployment using Streamlit/Multiple disease/trained_model.sav",'rb'))
load_heart=pickle.load(open("D:/Deployment using Streamlit/Multiple disease/trained_heart.sav",'rb'))
load_breast=pickle.load(open("D:/Deployment using Streamlit/Multiple disease/trained_breast.sav",'rb'))

with st.sidebar:
    selected=option_menu('Multiple Disease Prediction',['Diabetes Prediction','Heart Disease Prediction','Breast Cancer Prediction'],icons=['activity','heart','cast'],default_index=0)
...
if (selected=="Diabetes Prediction"):
    ...
    if st.button('Diagnosis'):
        diab_diagnosis=load_diabetes.predict([[Pregnancies,Glucose,BloodPressure,SkinThickness,Insulin,BMI,DiabetesPedigreeFunction,Age]])
...
if (selected=="Heart Disease Prediction"):
    ...
    if st.button('Diagnosis'):
        heart_diagnosis=load_heart.predict([[age,sex,cp,trestbps,chol,fbs,restecg,thalach,exang,oldpeak,slope,ca,thal]])
...
if (selected=="Breast Cancer Prediction"):
    ...
    if st.button('Diagnosis'):
        breast_diagnosis=load_breast.predict([[radius_mean,texture_mean,perimeter_mean,area_mean,smoothness_mean,compactness_mean, concavity_mean,concave_points_mean,symmetry_mean,fractal_dimension_mean,radius_se,texture_se,perimeter_se,area_se,smoothness_se,compactness_se,concavity_se,concave_points_se,symmetry_se,fractal_dimension_se,radius_worst,texture_worst,perimeter_worst,area_worst,smoothness_worst,compactness_worst,concavity_worst,concave_points_worst,symmetry_worst,fractal_dimension_worst]])
